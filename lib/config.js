...
// ── Lending / money-market (DEVNET only) ─────────────────────────────────────
// The REAL "SOL collateral → borrow PLSX" money-market deployed via Solana
// Playground (see anchor/pulse_lend/src/lib.rs + docs/LEND_DEPLOY.md). PUBLIC
// on-chain ids only — never secrets:
//   • LEND_PROGRAM_ID — the deployed program's address (devnet), keyless-verified.
//   • LEND_AUTHORITY  — the ONLY wallet that initializes the market + seeds/withdraws
//     the PLSX lending pool + tunes risk params. The site shows the lend admin panel
//     ONLY to this exact pubkey; every admin action is a plain transaction signed by
//     that wallet (no embedded key). Regular users only deposit SOL, borrow PLSX,
//     repay, and withdraw. Reused from staking/swap — the same wallet holds PLSX + SOL.
// The price ORACLE is our OWN swap pool (SWAP_PROGRAM_ID's pool for this mint),
// pinned into the market at initialize time and re-verified on-chain every borrow /
// withdraw / liquidate, so it can't be spoofed. Lend stays gated behind
// DEVNET_CONFIG.features.lend (still false) until the market is deployed, seeded,
// AND verified on-chain — nothing fake ships.
export const LEND_PROGRAM_ID = "6wkZxHJqAd7BywBLABhwpDV3au4hmWXuLWmu8Pi5yPPi";
export const LEND_AUTHORITY = "hU7g65F7jkyryGGWXSYTE4SY6k94NeQYfnYkveuSvvN";
